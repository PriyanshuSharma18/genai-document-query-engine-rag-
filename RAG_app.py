...
st.session_state.vectorstore_type = "Chroma"

# Initialize chain/memory/retriever defaults
...
def sidebar_and_documentChooser():
    ...
    with st.sidebar:
        ...
        if llm_chooser == list_LLM_providers[3]:
            expander_model_parameters(
                LLM_provider="Groq",
                text_input_API_key="Groq API key - [Get an API key](https://console.groq.com/keys)",
                list_models=["llama-3.3-70b-versatile", "llama-3.1-8b-instant", "mixtral-8x7b-32768"],
            )

        st.divider()
        st.subheader("Vectorstore")
        st.session_state.vectorstore_type = st.radio(
            "Select Vectorstore", ["Chroma", "Pinecone"]
        )
        if st.session_state.vectorstore_type == "Pinecone":
            st.session_state.pinecone_api_key = st.text_input(
                "Pinecone API Key", type="password", placeholder="insert your API key"
            )
            st.session_state.pinecone_index_name = st.text_input(
                "Pinecone Index Name", placeholder="e.g. rag"
            )
    ...
    with tab_open_vectorstore:
        ...
        if clicked:
            ...
            if (
                ...
                and not st.session_state.groq_api_key
            ):
            ...
            # if API keys are inserted, start loading Chroma index, then create retriever and ConversationalRetrievalChain
                ...
                if selected_vectorstore_path == "":
                    ...
                    with st.spinner("Loading vectorstore..."):
                        ...
                        try:
                            # 1. load Chroma or Pinecone vectorestore
...
def get_response_from_LLM(prompt):
    ...
    try:
        if st.session_state.chain is None:
            st.warning("Please create or open a vectorstore first before chatting.")
            return
        # 1. Invoke LLM
